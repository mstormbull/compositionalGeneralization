...
class TransformerModelWithCausal(nn.Module):
    def __init__(self, d_model, device, outputdim=1, inputdim=1,
                 num_heads=4, num_layers=2, seq_length=200, dropout=0.1, seed=0,
                 causal: bool = True): # Added causal toggle
        super().__init__()
        torch.manual_seed(seed)
        ...
        self.device = device
        self.seq_length = seq_length # Used for PositionalEncoding max_len
        self.causal = causal # Store the causal toggle
    ...
    def forward(self, x_seq):
        ...
        # Use is_causal flag in TransformerEncoder's forward pass
        # This requires PyTorch 1.9+ for is_causal in nn.TransformerEncoder.forward
        # For older versions, a manual mask would be needed:
        # src_mask = None
        # if self.causal:
        #     seq_len = pos_encoded_input.size(1)
        #     src_mask = nn.Transformer.generate_square_subsequent_mask(seq_len).to(pos_encoded_input.device)
        # encoded_seq = self.transformer_encoder(pos_encoded_input, mask=src_mask)

        # Using the cleaner is_causal flag (Pytorch 1.12+ recommended for full support, 1.9+ has it)
        encoded_seq = self.transformer_encoder(pos_encoded_input, is_causal=self.causal)
